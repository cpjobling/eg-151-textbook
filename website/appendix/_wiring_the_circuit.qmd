...
- For the analogue input, this can be connected to any of the GPIO pins which is also connected to the ADC.
  - Looking at @fig-ccode-1, these connections are labelled `A0` &ndash; `A7` on the nano board. Connection `A0` is connected to `PC0`, i.e. Port C bit 0 which is also connected to ADC[0] &ndash; ADC channel 0.
- The LEDs can be connected to any digital GPIO pin, but ideally will be 8 pins on the same port.
  - Looking at @fig-ccode-1 and the two pins we have used above, connections `D0` &ndash; `D7` can be used which corresponds to Port D bits 0-7[^note3].
 
A possible assembly is illustrated in @fig-ccode-2[^note4]
