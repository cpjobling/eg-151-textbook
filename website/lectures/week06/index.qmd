...
#### A bit t ADC has $2^3 = 8$ quantization levels (@fig-wk6-3bi).
...
Consider the signal with frequency $F_c = 1$Hz, sampling frequency of $F_s = 1$Hz, and resolution = 3 bits. The sampled signal is illustrated in @fig-wk6-violation`. Nyquist-Shannon sampling theorem has clearly been violated and the original signal cannot be reconstructed from the sampled signal.
...
The architecture of the Atmel ATmega328 is shown in @fig-wk6-atmel_adc`. It contains the following major components:
...
Consider the schemeatic diagram shown in @fig-wk6-sar_converter`.
...
## The Atmel ATmega328 Analog-digital-converter{#sec-wk7-sect3}
...
The value of these bits selects which analogue inputs are connected to the ADC (see @fig-wk6-admux_mux` and {numref}`wk6:table2`).
...
The ADC Data Registers are illustrated in @fig-wk6-adc_data`.
...
## Analogue I/O Example program{#sec-wk5-sect4}
